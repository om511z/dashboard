...
import { newJacket,getAdminProducts,clearErrors,deleteJacket } from '../../actions/jacketAction';
import { NEW_JACKET_RESET,DELETE_JACKET_RESET } from '../../constants/jacketConstants';
import { Button, Form, Input, message, Modal, Select, Table } from "antd";

const Jacket = ({history}) => {
    ...
    const { loading, error, success } = useSelector(state => state.newJacket);
    const {error:deleteError,isDeleted} = useSelector(state =>state.deletejacket)
    ...
    useEffect(() => {
        ...
        if (success) {
            ...
            dispatch({ type: NEW_JACKET_RESET })
        }
        if(deleteError) {
        ...
        if(isDeleted) {
           ...
           history.push('/jacket');
           dispatch({type: DELETE_JACKET_RESET })
