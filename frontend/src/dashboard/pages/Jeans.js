...
import { newJeans, getAdminProducts, clearErrors, deleteJeans } from '../../actions/jeansActions';
import { DELETE_JEANS_RESET, NEW_JEANS_RESET } from '../../constants/jeansConstants';
...
const Jeans = ({ history }) => {
    ...
    const { loading, error, success } = useSelector(state => state.newJeans);
    const {error:deleteError,isDeleted} = useSelector(state =>state.deletejeans)
    ...
    useEffect(() => {
        ...
        if(isDeleted) {
           ...
           history.push('/jacket');
           dispatch({type: DELETE_JEANS_RESET })
